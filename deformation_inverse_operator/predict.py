"""Prediction module for Deformation Inverse Operator (Route B).

Usage:
    import deformation_inverse_operator.predict as P

    # Quick predict from image arrays
    u = P.predict_dense(ref_img, tar_img, ckpt="checkpoints/route_b/best.pt")

    # Or use the Predictor class for repeated inference
    pred = P.Predictor("checkpoints/route_b/best.pt")
...
from .model import InverseOperatorModel
from .config import InverseOperatorConfig


class Predictor:
    """Deformation Inverse Operator predictor (Route B).
    ...
    def __init__(self, ckpt: Union[str, Path], device: str = "cuda"):
        ...
        config = state.get("config", InverseOperatorConfig())
        self.model = InverseOperatorModel(config).to(self.device)
    ...
    def dense(
        ...
        f_ref, f_tar = self.model.encode(ref_t, tar_t)
        queries = _dense_query_grid(H, W, self.device)
        u = self.model.decode(queries, f_ref, f_tar)
    ...
    def sparse(
        ...
        f_ref, f_tar = self.model.encode(ref_t, tar_t)
        u = self.model.decode(q_t, f_ref, f_tar)
        return u.squeeze(0).cpu().numpy()

    # ------------------------------------------------------------------
    # Convenience: factorised interface (encode once, decode many)
    # ------------------------------------------------------------------

    def encode(self, ref_img: np.ndarray, tar_img: np.ndarray):
        """Return (f_ref, f_tar) feature tensors for reuse across query sets."""
        ref_t = _to_tensor(ref_img).to(self.device)
        tar_t = _to_tensor(tar_img).to(self.device)
        return self.model.encode(ref_t, tar_t)

    @torch.no_grad()
    def decode(self, query_points: np.ndarray, f_ref: torch.Tensor, f_tar: torch.Tensor) -> np.ndarray:
        """Decode from pre-computed feature maps."""
        q_t = torch.from_numpy(query_points.astype(np.float32)).unsqueeze(0).to(self.device)
        u = self.model.decode(q_t, f_ref, f_tar)
        return u.squeeze(0).cpu().numpy()
...
def predict_dense(
    ref_img: np.ndarray,
    tar_img: np.ndarray,
    ckpt: Union[str, Path] = "checkpoints/route_b/best.pt",
    device: str = "cuda",
) -> np.ndarray:
